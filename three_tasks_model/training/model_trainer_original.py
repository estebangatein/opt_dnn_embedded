...
def custom_loss(y_true, y_pred, weights=[1.0, 1.0, 1.0]):
    ...
    if valid_tasks > 0:
        ...
        print('error')
        return tf.constant(0.0)  # O or small value for stability
...
def custom_mse_loss(y_true, y_pred):
    ...
    if tf.shape(y_true_masked)[0] == 0:
        return tf.constant(1e6)  # huge value to indicate error
...
sample_weight_task1_train = np.logical_not(np.isnan(np.sum(y_col_train, axis=1))).astype(np.float32) 
sample_weight_task2_train = np.logical_not(np.isnan(np.sum(y_fire_train, axis=1))).astype(np.float32) 
sample_weight_task3_train = np.logical_not(np.isnan(y_steer_train)).astype(np.float32) 
...
sample_weight_task1_val = np.logical_not(np.isnan(np.sum(y_col_val, axis=1))).astype(np.float32) 
sample_weight_task2_val = np.logical_not(np.isnan(np.sum(y_fire_val, axis=1))).astype(np.float32) 
sample_weight_task3_val = np.logical_not(np.isnan(y_steer_val)).astype(np.float32) 
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current_loss < self.best - self.min_delta:
            ...
            if self.wait >= self.patience:
                # restore weights with the best validation loss
...
model.compile(optimizer='adam',
              loss={'labels_output': custom_categorical_crossentropy_loss, 'fire_output':custom_categorical_crossentropy_loss, 'steering_output': custom_mse_loss}, weighted_metrics = {'labels_output': 'accuracy', 'fire_output':'accuracy', 'steering_output': 'mse'})


history = model.fit(x = images_train, 
          y = {'labels_output':y_train_task1, 'fire_output':y_train_task2, 'steering_output': y_train_task3}, 
          sample_weight={'labels_output':sample_weight_task1_train, 'fire_output':sample_weight_task2_train, 'steering_output': sample_weight_task3_train}, 
          epochs=100, 
          batch_size=32, 
          validation_data = (images_val, 
                             {'labels_output':y_val_task1, 'fire_output':y_val_task2, 'steering_output': y_val_task3}, 
                             {'labels_output':sample_weight_task1_val, 'fire_output':sample_weight_task2_val, 'steering_output': sample_weight_task3_val}), 
